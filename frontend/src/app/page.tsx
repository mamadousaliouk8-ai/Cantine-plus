...
export default function LoginPage() {
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(135deg, #1E5C30 0%, #328A4A 50%, #2A7A40 100%)",
      }}
    >
      {/* Language Selector */}
        ...
        >
          {(["login", "register"] as const).map((t) => (
            <button
              ...
              style={{
                background: tab === t ? "#F47B20" : "transparent",
                color: "white",
              }}
            >
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div>
                ...
                {[
                  ...
                  <button
                    ...
                    className={`p-3 rounded-xl border-2 transition-all font-medium text-sm ${
                      regRole === r.id
                        ? "border-[#F47B20] bg-[#F47B20]/10 text-white"
              ...
              <div className="mt-2 text-center">
                ...
                <span className="text-xs text-[#F47B20] font-black uppercase">
                  {regRole}
                </span>
              </div>
            </div>
            <InputField
              label="Nom / Prénom ou nom de la structure"
              type="text"
              value={regName}
              onChange={setRegName}
            ...
            <SubmitButton
              loading={loading}
              label={`Créer mon compte ${regRole}`}
            />
          </form>
        )}
      </div>
...
function SubmitButton({ loading, label }: { loading: boolean; label: string }) {
  return (
    <button
      ...
      style={{
        background: loading ? "#888" : "#F47B20",
        cursor: loading ? "not-allowed" : "pointer",
      }}
      onMouseEnter={(e) => {
        if (!loading)
          (e.target as HTMLButtonElement).style.background = "#1E5C30";
      }}
      onMouseLeave={(e) => {
        if (!loading)
          (e.target as HTMLButtonElement).style.background = "#F47B20";
      }}
    >
      {loading ? (
